...
test('affiche les nouvelles cartes et barres du dashboard', () => {
  ...
  assert.doesNotMatch(html, /securityCenter\.showScanHistoryPage/);
...
test('utilise une petite action Relancer dans l’en-tête du dashboard complet', () => {
  ...
  assert.match(sidebarHtml, /body\.surface-sidebar \.hero \{ display: grid !important/);
  assert.match(sidebarHtml, /body\.surface-sidebar \.pipeline-panel \{ display: block !important/);
...
test('sépare le dashboard en pages dédiées sans dupliquer les détails visibles', () => {
  ...
  assert.match(full, /Explorer Security Center/);
