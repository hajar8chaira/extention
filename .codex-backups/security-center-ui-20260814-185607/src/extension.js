...
class DashboardProvider {
  constructor(onCommand, themeController) {
    this.view = undefined;
  ...
  setData(findings, scanners, options) {
    this.model = buildDashboardModel(findings, scanners, options);
    this.render();
  }
  render() {
...
async function activate(context) {
  ...
  const dashboardProvider = new DashboardProvider((command, ...args) => vscode.commands.executeCommand(command, ...args), themeController);
  let currentFindings = [];
  ...
  status.command = 'securityCenter.scanWorkspace'; status.text = '$(shield) Security Center'; status.tooltip = 'Lancer une analyse de sécurité'; status.show();
  ...
  if (restoredScan) {
    ...
    status.text = activeCount ? `$(history) ${activeCount} vulnérabilité(s) restaurée(s)` : '$(history) Dernier scan restauré';
  ...
  context.subscriptions.push(vscode.commands.registerCommand('securityCenter.clearFindings', async () => {
    ...
    currentSecuritySnapshot = snapshotFromLegacy(); currentFindings = []; currentScanStatuses = []; currentDashboardOptions = {}; status.text = '$(shield) Security Center';
  ...
  context.subscriptions.push(vscode.commands.registerCommand('securityCenter.showTrends', async () => {
    try {
      ...
      const report = buildTrendReport(scans, events, 90);
      ...
      const nonce = crypto.randomBytes(16).toString('base64');
      const renderTrends = () => { panel.webview.html = renderTrendReportHtml(report, nonce, themeController.getTheme()); };
      renderTrends();
  ...
  context.subscriptions.push(vscode.commands.registerCommand('securityCenter.compareScans', async () => {
    try {
      ...
      if (scans.length < 2) return vscode.window.showInformationMessage('Security Center : au moins deux scans sont nécessaires pour comparer.');
      const currentChoice = await vscode.window.showQuickPick(scans.map((scan) => ({
        label: `Scan #${scan.scan_id} — ${scan.finding_count} résultat(s)`,
        description: new Date(scan.finished_at).toLocaleString('fr-FR'),
        detail: scan.workspace,
        scan
      })), { title: 'Comparer les scans', placeHolder: 'Choisir le scan le plus récent (après)' });
      if (!currentChoice) return;
      const baselineChoice = await vscode.window.showQuickPick(scans.filter((scan) => scan.scan_id !== currentChoice.scan.scan_id).map((scan) => ({
        label: `Scan #${scan.scan_id} — ${scan.finding_count} résultat(s)`,
        description: new Date(scan.finished_at).toLocaleString('fr-FR'),
        detail: scan.workspace,
        scan
      })), { title: `Comparer avec le scan #${currentChoice.scan.scan_id}`, placeHolder: 'Choisir le scan de référence (avant)' });
      if (!baselineChoice) return;
      const [first, second] = await Promise.all([
        getScan(baseUrl, baselineChoice.scan.scan_id),
        getScan(baseUrl, currentChoice.scan.scan_id)
      ]);
      const baseline = first.scan_id < second.scan_id ? first : second;
      const current = first.scan_id < second.scan_id ? second : first;
      const comparison = compareScans(baseline, current);
      const panel = vscode.window.createWebviewPanel(
        'securityCenter.scanComparison',
        `Security Center — scans #${baseline.scan_id} → #${current.scan_id}`,
        vscode.ViewColumn.Active,
        { enableScripts: false }
      );
      panel.webview.html = renderScanComparisonHtml(comparison, crypto.randomBytes(16).toString('base64'));
    } catch (error) {
  ...
  context.subscriptions.push(vscode.commands.registerCommand('securityCenter.showScanHistory', async () => {
    try {
      ...
      if (action.value === 'compare') {
        const candidates = scans.filter((scan) => scan.scan_id !== selected.scan.scan_id);
        if (!candidates.length) return vscode.window.showInformationMessage('Security Center : un second scan est nécessaire pour comparer.');
        const baselineChoice = await vscode.window.showQuickPick(candidates.map((scan) => ({
          label: `Scan #${scan.scan_id} — ${scan.finding_count} résultat(s)`,
          description: new Date(scan.finished_at).toLocaleString('fr-FR'),
          detail: scan.workspace,
          scan
        })), { title: `Comparer le scan #${selected.scan.scan_id}`, placeHolder: 'Choisir le scan de référence (avant)' });
        if (!baselineChoice) return;
        const [first, second] = await Promise.all([
          getScan(baseUrl, baselineChoice.scan.scan_id),
          getScan(baseUrl, selected.scan.scan_id)
        ]);
        const baseline = first.scan_id < second.scan_id ? first : second;
        const current = first.scan_id < second.scan_id ? second : first;
        const comparison = compareScans(baseline, current);
  ...
  context.subscriptions.push(vscode.commands.registerCommand('securityCenter.scanWorkspace', async (requestedTools) => {
    ...
    status.text = '$(loading~spin) Scan de sécurité…';
    try {
      ...
      try {
        ...
        const runScanner = async (scan, index) => {
          ...
          try {
            ...
            status.text = `$(loading~spin) Analyse ${scan.tool}…`;
        ...
        try {
          const storedScan = await saveScanResult(cfg.get('backend.url', 'http://127.0.0.1:8765'), {
            ...
            correlations: correlated.correlations
          });
        ...
        const activeCount = currentFindings.filter(isActiveFinding).length;
        if (cancelled) status.text = '$(circle-slash) Scan partiel — analyse annulée';
        else status.text = activeCount ? `$(shield) ${activeCount} vulnérabilité(s) active(s)` : '$(pass) Aucun résultat actif';
        ...
        status.text = '$(error) Échec du scan';
        const projection = projectSnapshot(currentSecuritySnapshot);
        currentFindings = projection.findings;
        currentScanStatuses = projection.scanners;
