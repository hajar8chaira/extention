...
<head>
  ...
  <style nonce="${nonce}">
    * { box-sizing: border-box; }
    ...
    body.surface-sidebar > h3.sidebar-keep { display: block !important; margin-top: 16px; }
    body.surface-sidebar .hero { display: grid !important; grid-template-columns: 92px minmax(0, 1fr); padding: 14px; }
    body.surface-sidebar .risk-ring { width: 82px; height: 82px; }
    body.surface-sidebar .risk-copy { min-width: 0; }
    body.surface-sidebar .risk-label { font-size: 15px; }
    body.surface-sidebar .risk-explanation { font-size: 10px; }
    body.surface-sidebar .pipeline-panel { display: block !important; padding: 12px 8px; }
    body.surface-sidebar .pipeline { min-width: max-content; gap: 6px; }
    body.surface-sidebar .pipeline-stage { min-width: 72px; }
    body.surface-sidebar .pipeline-line { width: 42px; min-width: 42px; }
    ...
    body.surface-full .hero { border-color: color-mix(in srgb, var(--vscode-widget-border) 80%, var(--vscode-button-background) 20%); padding: 20px; margin-top: 4px; background: linear-gradient(110deg, color-mix(in srgb, var(--vscode-button-background) 7%, transparent), transparent 58%); }
...
<body class="surface-${escapeHtml(surface)} theme-${selectedTheme === 'dark' ? 'dark' : 'light'}">
  ...
  ${surface === 'full' ? `<h3>Priority Findings ${resultsAvailable ? `<button class="quiet-action" data-command="securityCenter.openFindingsPage">Voir les ${priorityFindingCount} priorité${priorityFindingCount > 1 ? 's' : ''} →</button>` : ''}</h3><div class="priority-findings">${priorityFindings}</div><h3>Explorer Security Center</h3><div class="overview-pages"><div class="overview-link"><div><strong>Findings</strong><span>Filtres, preuves, triage et corrections IA.</span></div><button class="secondary" data-command="securityCenter.openFindingsPage">Voir tout →</button></div><div class="overview-link"><div><strong>Analyse dynamique</strong><span>ZAP, capture Burp et scénarios HTTP.</span></div><button class="secondary" data-command="securityCenter.openDynamicPage">Ouvrir →</button></div><div class="overview-link"><div><strong>Analytics</strong><span>Répartitions, corrections et corrélations.</span></div><button class="secondary" data-command="securityCenter.openAnalyticsPage">Ouvrir →</button></div></div>` : ''}
